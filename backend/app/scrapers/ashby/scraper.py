# backend/app/scrapers/ashby/scraper.py

from app.scrapers.base_scraper import BaseScraper
from app.scrapers.ashby.parser import AshbyParser
from app.utils.role_normalizer_v2 import normalize_title
from typing import List, Dict, Optional
import requests


class AshbyScraper(BaseScraper):
    """Scraper for Ashby ATS"""
    
    def __init__(self, verbose: bool = False):
        super().__init__()
        self.parser = AshbyParser()
        self.api_base_template = "https://api.ashbyhq.com/posting-api/job-board/{company}"
        self.verbose = verbose
    
    def get_company_jobs(self, company_slug: str) -> List[Dict]:
        """Fetch all jobs for a company using Ashby API"""
        self.rate_limit()
        
        api_url = self.api_base_template.format(company=company_slug)
        
        try:
            response = self.session.get(api_url, timeout=15)
            response.raise_for_status()
            
            data = response.json()
            
            # Ashby returns jobs in a 'jobs' array
            raw_jobs = data.get('jobs', [])
            
            print(f"📋 Found {len(raw_jobs)} jobs for {company_slug}")
            
            if not raw_jobs:
                return []
            
            normalized_jobs = []
            failed_count = 0
            
            for i, raw_job in enumerate(raw_jobs):
                try:
                    normalized = self.normalize_job(raw_job, company_slug)
                    normalized['company_slug'] = company_slug
                    normalized_jobs.append(normalized)
                    
                except Exception as e:
                    failed_count += 1
                    if self.verbose:
                        print(f"  ⚠ Error processing job {raw_job.get('id')}: {e}")
                    continue
                
                # Progress indicator every 50 jobs
                if (i + 1) % 50 == 0:
                    print(f"  Processed {i + 1}/{len(raw_jobs)}...")
            
            # Final summary
            status = "✅" if failed_count == 0 else "⚠️"
            print(f"{status} Fetched {len(normalized_jobs)} jobs from {company_slug}" +
                  (f" ({failed_count} failed)" if failed_count > 0 else ""))
            
            return normalized_jobs
            
        except requests.exceptions.HTTPError as e:
            if e.response.status_code == 404:
                print(f"❌ Company not found: {company_slug} (invalid slug?)")
            else:
                print(f"❌ HTTP error for {company_slug}: {e}")
            return []
        except requests.RequestException as e:
            print(f"❌ Failed to fetch jobs for {company_slug}: {e}")
            return []
    
    def normalize_job(self, raw_job: Dict, company_slug: str) -> Dict:
        """Convert Ashby JSON to standard format"""
        standard = self.get_standard_schema()
        
        # Extract location (can be in different places)
        location_data = (
            raw_job.get('location') or 
            raw_job.get('locationName') or 
            raw_job.get('jobLocation', {}).get('name', '') or
            ''
        )
        location = self.parser.parse_location(location_data)
        
        # Department/team
        department = (
            raw_job.get('department') or 
            raw_job.get('team') or
            raw_job.get('departmentName') or
            ''
        )
        
        # Employment type
        employment_type = raw_job.get('employmentType', '') or raw_job.get('type', '')
        
        # Description
        description_html = raw_job.get('descriptionHtml', '') or raw_job.get('description', '')
        description_text = self.parser.html_to_text(description_html)
        
        # If no HTML description, use plain text
        if not description_html and raw_job.get('descriptionPlain'):
            description_html = f"<p>{raw_job['descriptionPlain']}</p>"
            description_text = raw_job['descriptionPlain']
        
        # Build job URL
        job_id = raw_job.get('id', '')
        source_url = raw_job.get('jobUrl') or raw_job.get('applyUrl') or f"https://jobs.ashbyhq.com/{company_slug}/{job_id}"
        
        # Normalize the job title
        title = raw_job.get('title', '')
        role_info = normalize_title(title)
        
        # Posted date
        posted_at = self.parser.parse_date(
            raw_job.get('publishedAt') or 
            raw_job.get('createdAt') or
            raw_job.get('postedAt')
        )
        
        standard.update({
            'source_ats': 'ashby',
            'source_job_id': str(job_id),
            'source_url': source_url,
            'title': title,
            'location_raw': location['raw'],
            'location_city': location['city'],
            'location_state': location['state'],
            'location_country': location['country'],
            'location_is_remote': location['is_remote'],
            'department': department,
            'seniority_level': role_info['seniority_level'] or self.parser.infer_seniority(title),
            'employment_type': self.parser.parse_employment_type(employment_type),
            'description': description_html,
            'description_text': description_text,
            'posted_at': posted_at,
            'role_normalized_title': role_info['normalized_title'],
            'role_category': role_info['category'],
            'role_job_family': role_info['job_family'],
        })
        
        return standard
    
    def validate_company_slug(self, company_slug: str) -> Optional[int]:
        """
        Check if a company slug is valid and return job count.
        Returns None if invalid, job count if valid.
        """
        self.rate_limit()
        api_url = self.api_base_template.format(company=company_slug)
        
        try:
            response = self.session.get(api_url, timeout=10)
            if response.status_code == 200:
                data = response.json()
                jobs = data.get('jobs', [])
                return len(jobs)
            return None
        except requests.RequestException:
            return None